...
// 앞으로 K 칸을 점프하면 K 만큼의 건전지 사용량이 듭니다. 그러므로 아이언 슈트를 착용하고 
// 이동할 때는 순간 이동을 하는 것이 더 효율적입니다. 
...
// 이동할 수 있으므로 위치 2로 이동됩니다. 이때 다시 순간이동 하면 (현재까지 온 거리 : 2) x 2 만큼 이동할 수 있으므로 
...
// 입출력 예 #2
// 처음 위치 0 에서 1 칸을 앞으로 점프한 다음 순간이동 하면 (현재까지 온 거리 : 1) x 2에 해당하는 위치로 이동할 수 
// 있으므로 위치 2로 이동합니다. 이때 1 칸을 앞으로 점프하면 위치3으로 이동합니다. 
...
public class Ex011_JumpAndTeleport {
    ...
    public static int solution(int n) {
        ...
        // 사용한 배터리 양 저장 # Store battery usage
        int battery = 0;
        ...
        // 최소 배터리 사용량 반환 # Return minimum battery usage
        return battery;
    }
}