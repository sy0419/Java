...
// 상수는 순서에 맞게 쌓여서 완성된 햄버거를 따로 옮겨 포장을 하게 됩니다. 상수가 일하는 가게는 정해진 순서
...
public class Ex069_Hamburger { 
    ...
    public static int solution(int[] ingredient) {
        ...
        for (int ing : ingredient) { 
            stack.push(ing); // 재료 추가 # Push ingredient into the stack

            if (stack.size() >= 4) { // 햄버거 완성 가능 확인 # Check if a hamburger can be made
                int size = stack.size();
                // 마지막 4개의 재료가 [빵, 야채, 고기, 빵]인지 확인 # Check if last 4 ingredients are [1,2,3,1]
                if (stack.get(size-4) == 1 &&
                    stack.get(size-3) == 2 &&
                    stack.get(size-2) == 3 &&
                    stack.get(size-1) == 1) {
                    stack.pop(); stack.pop(); stack.pop(); stack.pop(); // 완성된 햄버거 제거 # Remove the hamburger
                    result++; // 햄버거 개수 증가 # Increase hamburger count
                }
            }
        }
